...
namespace DisplayQueryResult
{
   public partial class TitleQueries : Form
   {
         ...
         object sender, EventArgs e)
      {
        ...
        {
            //fetch the rows which contains the searched string title column from the title table

            var lastNameQuery = from title in dbcontext.Titles
                                where title.Title1.Contains(findTextBox.Text)
                                orderby title.Title1, title.Title1
                                select title;

            //Display the rows
            titleBindingSource.DataSource = lastNameQuery.ToList();
            //go to first result
            titleBindingSource.MoveFirst();

            //disable navigator to add new row
            bindingNavigatorAddNewItem.Enabled = false;

            //disable navigator to delete a row
